...
public class DBLogic
{
        ...
        using (SqlConnection connection = new SqlConnection(connectionString))
        {
            ...
            if(reader.HasRows) // если есть данные
            {
                ...
                //while (reader.Read()) // построчно считываем данные
                    //Users.Add(new Account{Nickname = (string)reader.GetValue(1), Password = (string)reader.GetValue(2)});
            }
    ...
    public static Account GetUserById(string connectionString,int id)
    {
        ...
        string sqlExpression = $"SELECT * FROM SteamDB.Accounts acc where acc.id = {id}";
        using (SqlConnection connection = new SqlConnection(connectionString))
        {
            connection.Open();
    ...
    public static void SaveUser(string connectionString,string nickname, string password)
    {
        ...
        string sqlExpression = "INSERT INTO SteamDB.Accounts VALUES (@nickname, @password)";
